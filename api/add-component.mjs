// Vercel Serverless Function: 一键添加效果组件（GitHub 桥接版）
// 流程：解析命令 -> 抓注册表 JSON -> 生成组件文件/卡片 -> GitHub API 提交 -> Vercel 自动重新构建部署
// 需要环境变量：GH_TOKEN（仓库写入权限的 GitHub PAT）
...
export default async function handler(req, res) {
  ...
  const owner = process.env.VERCEL_GIT_REPO_OWNER || process.env.GH_OWNER
  const repo = process.env.VERCEL_GIT_REPO_SLUG || process.env.GH_REPO
  const branch = process.env.GH_BRANCH || 'main'

  if (!token || !owner || !repo) {
    res.statusCode = 500
    res.end(
      '服务器缺少环境变量：GH_TOKEN / VERCEL_GIT_REPO_OWNER / VERCEL_GIT_REPO_SLUG（或 GH_OWNER / GH_REPO）。请先在 Vercel 项目设置中配置。'
    )
    return
  }
  ...
  req.on('end', async () => {
    ...
    if (!items) {
      res.statusCode = 400
      res.end('命令格式不合法：仅支持形如 npx shadcn@latest add @注册源/组件名（aceternity / react-bits / 20mhz / magicui / neobrutalism / motion-primitives），可逗号分隔多个。')
      return
    }
    ...
    try {
      ...
      const manifest = JSON.parse(Buffer.from(existing.content, 'base64').toString('utf8'))
      ...
      write(`\n✅ 已提交，Vercel 正在自动重新构建部署（约 1~2 分钟）。\n`)
